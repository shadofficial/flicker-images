...
export const fetchdefault = async (page=1) => {
    const responsed = await fetch(`https://www.flickr.com/services/rest/?method=flickr.photos.getRecent&api_key=1583ada955f818856c5692be7cb57c4e&per_page=12&page=${page}&format=json&nojsoncallback=1`);
    const jsond = await responsed.json();
    const datad = await jsond.photos;
    return datad.photo;
};
